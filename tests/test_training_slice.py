...
from diploma_pinn.config.schema import DataConfig, ExperimentConfig, ModelConfig, PhysicsConfig, RuntimeConfig
...
def test_factory_steps_plateau_scheduler_at_epoch_boundary(tmp_path: Path) -> None:
    path = tmp_path / "dns.npz"
    inputs = np.array([[time, 0.2, 0.3, 0.4] for time in (0.0, 0.5) for _ in range(4)], dtype=np.float32)
    outputs = np.zeros((len(inputs), 5), dtype=np.float32)
    np.savez(path, inputs=inputs, outputs=outputs)
    config = ExperimentConfig(
        experiment_id="scheduler-test",
        data=DataConfig(path=path, expected_sha256="", batch_size=4),
        model=ModelConfig(widths=(4, 8, 5)),
        runtime=RuntimeConfig(device="cpu", max_steps=2, scheduler_epoch_steps=1),
    )
    metrics = build_experiment(config).trainer.run()
    assert metrics.step == 1


def test_factory_runs_one_cpu_vv_update(tmp_path: Path) -> None:
    path = tmp_path / "dns.npz"
    inputs = np.array([[time, 0.2, 0.3, 0.4] for time in (0.0, 0.5) for _ in range(4)], dtype=np.float32)
    outputs = np.zeros((len(inputs), 5), dtype=np.float32)
    np.savez(path, inputs=inputs, outputs=outputs)
    config = ExperimentConfig(
        experiment_id="vv-test",
        data=DataConfig(path=path, expected_sha256="", batch_size=4),
        model=ModelConfig(widths=(4, 8, 4)),
        physics=PhysicsConfig(formulation="vv"),
        runtime=RuntimeConfig(device="cpu", max_steps=1),
    )
    metrics = build_experiment(config).trainer.run()
    assert metrics.step == 0
