...
def velocity_mse(model: nn.Module, observations: ObservationBatch) -> Tensor:
    ...
    if predicted.ndim != 2 or predicted.shape[0] != observations.points.shape[0] or predicted.shape[1] < 3:
        raise ValueError(f"expected model output [N,M] with M >= 3, got {tuple(predicted.shape)}")
